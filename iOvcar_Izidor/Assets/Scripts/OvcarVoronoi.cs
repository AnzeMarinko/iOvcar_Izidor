...
public class OvcarVoronoi : MonoBehaviour
{
    ...
    float casNakljucnegaPremika = 3f;
    ...
    public void VoronoiStart()
    {
        ...
        casDoNakljucnegaPremika = Random.Range(0f, 20f) + 60f;   // nakjucen premik naj bo ob nakljucnem casu cez slabo minuto
    ...
    public void VoronoiUpdate()
    {
        ...
        if (ovce.Length > 0)
        {
            ...
            foreach (GameObject ovca in ovce)
            {
                ...
                float mera = razdalja * razdalja / doOvce;  
                if (ovca.GetComponent<GinelliOvca>().voronoiPes == ovcar && razdalja > fN && (cilj - ovca.transform.position).magnitude > (GCM - cilj).magnitude - 2 && mera > doPobegle)
                {
            ...
            if (doCentra > fN && ovcarji.Length * ovcarji.Length * pobeglih > ovce.Length * 0.15f)   // ce je pobeglih ovc vec kot 15% / #ovcarjev^2
            {
                soZbrane = false;
            }
            if (ovce.Length < ovcarji.Length * 2)  // ko je ovc zelo malo (manj kot trikrat toliko kot ovcarjev) podaljsaj dolzino nakljucnega premika, da dajo psi ovcam vec prostora
            {
                casNakljucnegaPremika = 5 + ovcarji.Length * 5 - ovce.Length;
            }
            if (casDoNakljucnegaPremika < 0f)
            {
                casDoNakljucnegaPremika = Random.Range(0f, 20f) + 60f + casNakljucnegaPremika;
                nakljucenPremik = false;
                zacetekNakljucnegaPremika = true;
            }
            else if (casDoNakljucnegaPremika < casNakljucnegaPremika)
            {
            ...
            if (premikNazaj < 0)
            {
                smer = (ovcar.transform.position - cilj) * Mathf.Sqrt(- premikNazaj) + (tocka.transform.position - ovcar.transform.position) * 2f;
            }
            ...
            foreach (GameObject izbran in ovcarji)   // ovcarji naj se med seboj izogibajo in drzijo razdaljo
            {
                ...
                {
                    smer -= razdalja.normalized * 20f / (razdalja.magnitude + 1f);
                }
            }
            smer = smer.normalized;

            // ne zaokrozi, ce je skoraj na tocki
            if ((ovcar.transform.position - tocka.transform.position).magnitude < 12f ||
            ...
            foreach (GameObject o in ovcarji)
            {
                ...
                float oProtiOvcarju = razdalja.normalized.x * o.GetComponent<OvcarVoronoi>().smer.normalized.x +
                    razdalja.normalized.z * o.GetComponent<OvcarVoronoi>().smer.normalized.z;
            ...
            float phi = Random.Range(-Mathf.PI / 3f, e * Mathf.PI / 3f);  // dodaj sum
